export default function RaceTrack({ raceData, currentEvent }) {
  ...
  return (
    <div className="w-full glass-card bg-[#111111] p-4 relative border-t-8 border-b-8 border-gray-600 rounded-none mb-6">
      <div className="text-center mb-4 border-b border-white/10 pb-2">
        <h2 className="font-black text-white text-xl tracking-wider uppercase">🏁 {raceData.name || "Virtual Grand Prix"} 🏁</h2>
      ...
      <div className="relative border-l-4 border-white/50 border-r-4 border-red-500/80 rounded py-2 shadow-inner bg-black/20">
        ...
        {/* Track lines (Asphalt dashes) */}
        <div className="absolute top-0 bottom-0 left-[25%] border-l-[3px] border-white/40 border-dashed pointer-events-none"></div>
        <div className="absolute top-0 bottom-0 left-[50%] border-l-[3px] border-white/40 border-dashed pointer-events-none"></div>
        <div className="absolute top-0 bottom-0 left-[75%] border-l-[3px] border-white/40 border-dashed pointer-events-none"></div>
            ...
            return (
              <div key={runner.id} className="relative h-12 w-full bg-white/5 rounded overflow-hidden flex items-center border border-white/5">
                ...
                >
                  ...
                  <span className="text-2xl ml-1 filter drop-shadow-lg scale-x-[-1]">🏎️</span>
                  
                  {/* Exhaust flame/smoke if moving fast */}
                  {!finished && distance > 5 && distance < 95 && (
                    <div className="absolute right-full top-1/2 -translate-y-1/2 w-6 h-3 bg-gradient-to-r from-orange-500 to-transparent opacity-80 blur-sm rounded-full animate-pulse transform -translate-x-1"></div>
                  )}
                </div>
